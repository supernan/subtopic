...
#define NONE  0//文档尚未被simhash处理
#define NO_DUPLICATE 1 //标记文档不重复
#define DUPLICATE -1 //标记文档重复
namespace nlp
{
	...
	class Plsa
	{
		...
		private:
			...
			int _iter_nums; //迭代次数
			int _thread_nums; //分词线程数
			int _partial_nums; //采样文档比例 如取值为n 文档采样比例为 1/n
			int _max_docs; //最大文档数，大于此阈值 需要进行采样
			std::map<int, double> _doc_words_map; //统计每篇文档中的总词汇数
			std::map<int, WeiboTopic_ICT::Weibo> _id_doc_map; //文档id与文档对象映射关系(采样)
			std::map<int, WeiboTopic_ICT::Weibo> _id_total_map; //文档id与文档对象映射关系(全部)
