...
bool nlp::Plsa::_load_config(std::string &path)
{
	...
	{
		LOG(FATAL) << "Configuration file not found" << path << std::endl;
		return false;
	}
	...
	TiXmlElement *thread_node = doc_node->NextSiblingElement();
	TiXmlElement *partial_node = thread_node->NextSiblingElement();
	TiXmlElement *maxdoc_node = partial_node->NextSiblingElement();
	TiXmlElement *ict_node = maxdoc_node->NextSiblingElement();
	...
	std::string thread_str = thread_node->FirstChild()->Value();
	std::string partial_str = partial_node->FirstChild()->Value();
	std::string maxdoc_str = maxdoc_node->FirstChild()->Value();
	std::string ict_str = ict_node->FirstChild()->Value();

	_topics_nums = atoi(topics_str.c_str());
	_iter_nums = atoi(iters_str.c_str());
	_thread_nums = atoi(thread_str.c_str());
	_partial_nums = atoi(partial_str.c_str());
	_max_docs = atoi(maxdoc_str.c_str());
	...
	delete plsa_config;
	if (_topics_nums > MAX_TOPICS)
	{
		LOG(FATAL) << "Topics is beyound MAX_TOPICS" << std::endl;
		return false;
	}
		                ...
		                std::map<std::string, int> &tmp_tf_map)
{
    ...
    tools::WordSegThreadPool wordseg_pool(_ict_conf, _thread_nums, _id_doc_map);
...
int nlp::Plsa::_build_simhash(std::vector<WeiboTopic_ICT::Weibo> &doc_list, std::vector<int> &positions) //fix here
{
	...
	Simhash::Open(1, _simhash_handler);
	...
	for (unsigned int i = 0; i < texts.size(); ++i)
	{
		...
		else
		{
			...
			for (unsigned j = 0; j < index.size(); ++j)
				if (index[j] != i && positions[index[j]] == NONE)
					positions[index[j]] = DUPLICATE;
		}
	}
...
void nlp::Plsa::save_probs(std::string &path)
{
	...
	std::map<int, WeiboTopic_ICT::Weibo>::iterator wit;
	for (wit = _id_total_map.begin(); wit != _id_total_map.end(); ++wit)
	{
		int did = wit->first;
		string text = wit->second.mt;
		text_out<<did<<" "<<text<<endl;
	}
	text_out.close();
	term_out.close();
	doc_out.close();
...
void nlp::Plsa::set_documents(std::vector<WeiboTopic_ICT::Weibo> &doc_list)
{
	_id_doc_map.clear();
	_id_total_map.clear();
	std::vector<int> positions(doc_list.size(), NONE);
	std::cout<<"befor hash"<<doc_list.size()<<std::endl;
	//int unique_count =_build_simhash(doc_list, positions);
	int unique_count = doc_list.size();
	//std::cout<<"unique_count: "<<unique_count<<endl;
	int count = 0;
	int total_count = 0;
	for (unsigned int i = 0; i < doc_list.size(); i++)
	{
		/*if (positions[i] == NO_DUPLICATE)
		{
			...
			if (unique_count > _max_docs)
			{
				//if (rand() % _partial_nums > 0)
				//	sample_flg = false;
			}
			if (sample_flg)
			{
		...
		_id_total_map[total_count] = doc_list[i];
		_id_doc_map[total_count] = doc_list[i];
		total_count += 1;
	}
